...
import com.appspot.trak.shipment.model.StartShipmentRequest;
...
public class StartShipmentDeliveryActivity extends Activity {
    ...
    private void startShipmentDeliveryProcess() {
        checkNfc();
        if (nfcAdapter == null) {
            return;
        }
        ...
        if (NfcAdapter.ACTION_NDEF_DISCOVERED.equals(action)) {
            if (canRead) {
                ...
                if (App.MIME_TEXT_PLAIN.equals(type)) {
                    ...
                    new StartShipmentDeliveryTask(this).execute(tag);
    ...
    private class StartShipmentDeliveryTask extends AsyncTask<Tag, Void, Boolean> {

        private Context context;
        private ProgressDialog progressDialog;

        private StartShipmentDeliveryTask(Context context) {
        ...
        protected Boolean doInBackground(Tag... tags) {
            try {
                ...
                for (NdefRecord ndefRecord : records) {
                    if (ndefRecord.getTnf() == NdefRecord.TNF_WELL_KNOWN && Arrays.equals(ndefRecord.getType(), NdefRecord.RTD_TEXT)) {
                        try {
                            ...
                            StartShipmentRequest startShipmentRequest = new StartShipmentRequest();
                            startShipmentRequest.setShipmentId(Long.parseLong(shipmentId));
                            startShipmentRequest.setUserId(Long.parseLong(userId));
                            shipmentService.startShipmentDelivery(startShipmentRequest).execute();
                            return Boolean.TRUE;
                ...
                StartShipmentDeliveryActivity.this.runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        AlertDialog.Builder builder = new AlertDialog.Builder(StartShipmentDeliveryActivity.this);
        ...
        protected void onPostExecute(Boolean result) {
            ...
            if (result) {
                StartShipmentDeliveryActivity.this.finish();
            }
        }
